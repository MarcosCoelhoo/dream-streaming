...
export default async function initCreateTopRatedSerie() {
  ...
  for (let page = 1; page <= numPages; page++) {
    const response = await (
      await fetch(
        `${baseUrl}/tv/top_rated?api_key=${apiKey}&page=${page}&language=en`
      )
    ).json();
...
<div class="movie-content">
  ...
  carouselContent("#serie-top-rated");
}
